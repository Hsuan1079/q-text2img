...
def check_gpu_usage(threshold=80):
    try:
        pynvml.nvmlInit()
        handle = pynvml.nvmlDeviceGetHandleByIndex(0)  # 假設你使用第0張卡
        mem_info = pynvml.nvmlDeviceGetMemoryInfo(handle)
        used_percent = mem_info.used / mem_info.total * 100
        if used_percent > threshold:
            logger.warning(f"⚠️ GPU 使用率已達 {used_percent:.2f}%，超過 {threshold}% 限制，將自動停止程式")
            pynvml.nvmlShutdown()
            return True
        pynvml.nvmlShutdown()
        return False
    except Exception as e:
        logger.warning(f"⚠️ 無法檢查 GPU 使用率：{e}")
        return False
...
def main():
    ...
    parser.add_argument(
        "--cali_data_path", type=str, default="sd_cali_steps.pt",
                    ...
                    for name, module in model.named_children():
                        ...
                        if isinstance(module, QuantModule):
                            if module.ignore_reconstruction is True:
                                ...
                                if check_gpu_usage(80):
                                    logger.error("⛔ 停止重建：GPU 使用率過高")
                                    exit(1)
